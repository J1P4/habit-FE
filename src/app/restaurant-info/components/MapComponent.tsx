        ...
        <LoadScript googleMapsApiKey="AIzaSyCwrWwOutdytyZU67z3z5a9KmrewnqoCcc">
          <GoogleMap
            mapContainerStyle={{
              ...
              margin: 'auto',
              borderRadius: '10px',
              boxShadow: '0 4px 6px rgba(0, 0, 0.5, 0.2)',
            }}
            ...
            {localList?.map((local, index) => {
              ...
              return (
                <Marker
                  key={index}
                  position={{
                    lat: local?.latitude || DEFAULT_LATITUDE,
                    lng: local?.longitude || DEFAULT_LONGITUDE,
                  }}
                  title={local.title}
                  onClick={() => setSelectedMarkerIndex(index)}
                >
                  {selectedMarkerIndex === index && (
                    <InfoWindow onCloseClick={() => setSelectedMarkerIndex(null)}>
                      <div>
                        <h3>{local.title}</h3>
                      </div>
                    </InfoWindow>
